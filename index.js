...
// 🔴 यहाँ अपनी UltraMsg की Details डालें
const ULTRAMSG_INSTANCE_ID = "YOUR_INSTANCE_ID_HERE"; // उदा: instance105432
const ULTRAMSG_TOKEN = "YOUR_TOKEN_HERE";             // उदा: abc123xyz...
...
// 📩 UltraMsg से टेक्स्ट मैसेज भेजने का फ़ंक्शन
async function sendUltraMsgText(toPhone, messageText) {
    try {
        let formatted = toPhone.toString().replace(/[^0-9]/g, '');
        ...
        console.log(`✅ [UltraMsg TEXT] भेजा गया -> ${formatted}`);
        return response.data;
    } catch (err) {
        console.error('❌ UltraMsg Text Error:', err.message);
    }
}

// 📄 UltraMsg से PDF रसीद जनरेट करके भेजने का फ़ंक्शन
async function sendUltraMsgPdf(toPhone, data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.on('end', async () => {
                ...
                await axios.post(`https://api.ultramsg.com/${ULTRAMSG_INSTANCE_ID}/messages/document`, {
                    token: ULTRAMSG_TOKEN,
                    to: formatted,
                ...
                console.log(`✅ [UltraMsg PDF] भेजी गई -> ${formatted}`);
                resolve();
            });

            // 🎨 BRANDED SCHOOL RECEIPT DESIGN
...
// 🌐 UltraMsg Webhook Receiver (आने वाले मैसेजेस और ऑटो-रिप्लाई)
    ...
    try {
        ...
        if (event === 'message_received' && msg.from && !msg.from.includes('@g.us')) {
            ...
            if (isGreeting) {
                ...
                await sendUltraMsgText(senderPhone, menuText);
                return res.sendStatus(200);
            }
...
// 🎯 DUAL DELIVERY QUEUE ENGINE (GOOGLE SHEET ALERT TRIGGER)
let messageQueue = [];
...
async function processQueue() {
    ...
    while (messageQueue.length > 0) {
        ...
        try {
            ...
            // 1. टेक्स्ट मैसेज
            await sendUltraMsgText(item.number, textToSend);

            // 2. 1 सेकंड बाद PDF रसीद
            await new Promise(res => setTimeout(res, 1000));
            await sendUltraMsgPdf(item.number, item);

            messageQueue.shift();
            await new Promise(res => setTimeout(res, 1000));

        } catch (err) {
            console.error(`❌ Queue Error (${item.number}):`, err.message);
            messageQueue.shift();
        }
    }
...
app.post('/enqueue-message', (req, res) => {
    ...
    if (!targetPhone) return res.status(400).json({ status: 'error', message: 'Missing phone' });
...
app.get('/', (req, res) => res.send('UltraMsg WhatsApp Gateway is Active ✅'));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`UltraMsg Bot running on port ${PORT}`));
